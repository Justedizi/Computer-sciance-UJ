...
int main() {
	const char test_name[] = "symmetric/";
	...
	for(int i = 0; i < random_test_size; i = i + 1) {
		...
		int symmetric = set1 ^ set2;

		std::string correct_print = stringify_set(symmetric, false);
		...
		Symmetric(user_set1, user_set2, &user_set_res);
		Print(user_set_res, user_buffer);
		...
		if(!result) {
			...
			std::cerr << "[\033[33mDEBUG\033[0m]            SET2: >" << emplace_string1 << "<\n";
