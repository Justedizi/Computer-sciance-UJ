...
int main() {
	...
	for(int i = 0; i < random_test_size; i = i + 1) {
		...
		int set = randint();
		int to_erase = randint();

		//generate correct and shuffled output
		int erased = set & (~to_erase);

		std::string correct_print = stringify_set(erased, false);
		std::string emplace_string = stringify_set(set, false);
		std::string erase_string = stringify_set(to_erase, true);
		...
		Erase((char*)erase_string.c_str(), &user_set);
		...
		if(!result) {
			...
			std::cerr << "[\033[33mDEBUG\033[0m] THINGS TO ERASE: >" << erase_string << "<\n";
