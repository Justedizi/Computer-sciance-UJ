...
int main() {
	...
	for(int i = 0; i < random_test_size; i = i + 1) {
		...
		int unioned = set | to_insert;

		std::string correct_print = stringify_set(unioned, false);
		std::string emplace_string = stringify_set(set, false);
		std::string insert_string = stringify_set(to_insert, true);
		...
		Insert((char*)insert_string.c_str(), &user_set);
		...
		if(!result) {
			...
			std::cerr << "[\033[33mDEBUG\033[0m] THINGS TO INSERT: >" << insert_string << "<\n";
