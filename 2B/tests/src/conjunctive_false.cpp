...
int main() {
	const char test_name[] = "conjunctive_false/";

	//RANDOM TESTS
	bool rand_result = true;
	for(int i = 0; i < random_test_size; i = i + 1) {
		//generate random set for that test
		int root_set = randint();
		int mask_set1 = randint();
		int mask_set2 = ~mask_set1;

		int set1 = root_set & mask_set1;
		int set2 = root_set & mask_set2;
		...
		bool user_result = Conjunctive(user_set1, user_set2);
