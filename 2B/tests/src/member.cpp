...
int main() {
	...
	for(int i = 0; i < random_test_size; i = i + 1) {
		...
		int set = randint();
		int member_number = randint() & 31;

		//generate correct and shuffled output
		bool correct_result = (set & (1 << member_number));

		std::string emplace_string = stringify_set(set, false);
		std::string member_string = stringify_set((1 << member_number), true);
		...
		bool user_result = Member((char*)member_string.c_str(), user_set);
		...
		if(!result) {
			...
			std::cerr << "[\033[33mDEBUG\033[0m] CHECKING MEMBER: >" << stringify_set((1 << member_number), false) << "<\n";
